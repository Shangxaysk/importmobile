  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-black font-inter pb-32 p-6 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input placeholder={t('product_name_placeholder')} className="input-std" 
            ...
            <div className="mt-4">
                ...
                <textarea rows={4} className="input-std" placeholder={t('description_placeholder')}
                          value={basic.description} onChange={e => setBasic({...basic, description: e.target.value})}></textarea>
            </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-sm border border-gray-100 dark:border-gray-800 relative overflow-hidden transition-colors">
            ...
            <div className="flex flex-col md:flex-row gap-3 items-end mb-4 relative z-10">
                <div className="flex-1 w-full">
                    ...
                    <input type="number" placeholder="8" className="input-std" 
                           value={tempVar.ram} onChange={e => setTempVar({...tempVar, ram: e.target.value})}/>
                </div>
                <div className="flex-1 w-full">
                    <label className="spec-label">ROM (GB)</label>
                    <input type="number" placeholder="256" className="input-std" 
                           value={tempVar.rom} onChange={e => setTempVar({...tempVar, rom: e.target.value})}/>
                </div>
                <div className="flex-[2] w-full">
                    <label className="spec-label">{t('price')} ($)</label>
                    <input type="number" placeholder="1200" className="input-std" 
                           value={tempVar.price} onChange={e => setTempVar({...tempVar, price: e.target.value})}/>
                </div>
                <button onClick={addVariant} className="h-[52px] px-6 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl flex items-center justify-center transition shadow-lg shadow-emerald-500/20 active:scale-95">
                    <Plus strokeWidth={3}/>
                </button>
            </div>
                    ...
                    {variants.map((v, idx) => (
                        <div key={idx} className="flex items-center justify-between bg-emerald-50 dark:bg-emerald-900/10 p-3 rounded-xl border border-emerald-100 dark:border-emerald-900/20">
                            <div className="flex items-center gap-4 font-bold text-gray-700 dark:text-gray-200">
                                <span className="bg-white dark:bg-black px-3 py-1 rounded-lg shadow-sm border dark:border-gray-700">{v.ram}/{v.rom} GB</span>
                                <span className="text-emerald-700 dark:text-emerald-400 text-lg">{Number(v.price).toLocaleString()} $</span>
                            </div>
                            <button onClick={() => removeVariant(idx)} className="p-2 bg-white dark:bg-black text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 border border-transparent hover:border-red-200 transition"><X size={18}/></button>
                        </div>
                    ))}
                </div>
            )}
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
                <Palette className="text-emerald-600"/> {t('images_colors')} ({images.length}/7)
            </h2>
                ...
                {images.map((img, idx) => (
                    <div key={idx} className="relative group bg-gray-50 dark:bg-black rounded-2xl p-2 border border-gray-200 dark:border-gray-800">
                        ...
                        <div className="flex items-center gap-2">
                            <input type="color" value={img.color} onChange={e => updateImageColor(idx, e.target.value)} 
                                   className="w-8 h-8 rounded-full cursor-pointer border-2 border-white dark:border-gray-700 shadow-sm p-0 overflow-hidden" />
                            <input type="text" value={img.color} onChange={e => updateImageColor(idx, e.target.value)} 
                                   className="text-xs font-mono w-full bg-white dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 rounded-md px-1 py-1 uppercase" maxLength={7}/>
                        </div>
                    </div>
                ))}
        ...
        <div className="bg-slate-50 dark:bg-gray-900/50 p-6 rounded-[32px] border border-slate-200 dark:border-gray-800 transition-colors">
             ...
             <div className="grid md:grid-cols-2 gap-x-8 gap-y-6">
                 ...
                 <div>
                    ...
                    <input list="chip-list" className="input-std bg-white dark:bg-black" placeholder={t('chipset_placeholder')} 
                           value={specs.chipset} onChange={e => setSpecs({...specs, chipset: e.target.value})} />
                    <datalist id="chip-list">
                        {[...CONSTANTS.chips.android, ...CONSTANTS.chips.ios].map(c => <option key={c} value={c}/>)}
                    </datalist>
                 </div>
                 ...
                 <div className="md:col-span-2 bg-white dark:bg-black p-4 rounded-2xl border border-gray-200 dark:border-gray-800">
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                            <input list="os-list" className="input-std" placeholder={specs.osType === 'Android' ? t('os_android_placeholder') : t('os_ios_placeholder')}
                                   value={specs.osVer} onChange={e => setSpecs({...specs, osVer: e.target.value})} />
                            <datalist id="os-list">
                                {(specs.osType === 'Android' ? CONSTANTS.os.android : CONSTANTS.os.ios).map(os => <option key={os} value={os}/>)}
                            </datalist>
                        </div>
                    </div>
                 </div>
                 ...
                 <div className="md:col-span-2 grid md:grid-cols-2 gap-6 bg-white dark:bg-black p-4 rounded-2xl border border-gray-200 dark:border-gray-800">
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <select className="bg-gray-100 dark:bg-gray-900 dark:text-white text-xs font-bold p-1 rounded" value={specs.camRearCount} onChange={e => setSpecs({...specs, camRearCount: Number(e.target.value)})}>
                        ...
                        <div className="flex gap-2">
                            {Array.from({length: specs.camRearCount}).map((_, i) => (
                                <input key={i} type="number" placeholder={`${i+1}-MP`} className="input-std px-2 text-center"
                                       value={specs.camRearMPs[i] || ''} onChange={e => updateCamMP('rear', i, e.target.value)} />
                            ))}
                        </div>
                    </div>
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <select className="bg-gray-100 dark:bg-gray-900 dark:text-white text-xs font-bold p-1 rounded" value={specs.camFrontCount} onChange={e => setSpecs({...specs, camFrontCount: Number(e.target.value)})}>
                        ...
                        <div className="flex gap-2">
                            {Array.from({length: specs.camFrontCount}).map((_, i) => (
                                <input key={i} type="number" placeholder={`${i+1}-MP`} className="input-std px-2 text-center"
                                       value={specs.camFrontMPs[i] || ''} onChange={e => updateCamMP('front', i, e.target.value)} />
                            ))}
                        </div>
                    </div>
                 </div>
                 ...
                 <div>
                    ...
                    <div className="relative">
                        <input type="number" className="input-std bg-white dark:bg-black pr-12" placeholder="5000"
                 ...
                 <div>
                    ...
                    <div className="relative">
                        <input type="number" className="input-std bg-white dark:bg-black pr-12" placeholder="200"
                 ...
                 <div className="md:col-span-2">
                    ...
                    <input list="sim-list" className="input-std bg-white dark:bg-black" placeholder="Nano-SIM + eSIM..." 
                           value={specs.sim} onChange={e => setSpecs({...specs, sim: e.target.value})} />
                    <datalist id="sim-list">
                        <option value="Nano-SIM + eSIM" />
                        <option value="2x Nano-SIM" />
                        <option value="eSIM only" />
                        <option value="2x Nano-SIM + eSIM" />
                    </datalist>
                 </div>
             </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input placeholder="15-20" className="input-std" 
                           value={basic.deliveryTime} onChange={e => setBasic({...basic, deliveryTime: e.target.value})} />
                </div>
                <div>
                    <label className="spec-label">{t('prepayment_percent')}</label>
                    <div className="relative">
                        <input type="number" placeholder="100" max="100" className="input-std pr-10" 
                               value={basic.prepaymentPercent} onChange={e => setBasic({...basic, prepaymentPercent: e.target.value})} />
                        <Percent size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400"/>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1 ml-2">{t('prepayment_hint')}</p>
                </div>
            </div>
        </div>

        {/* SAVE BUTTON */}
        <button onClick={handleSubmit} disabled={uploading} className="w-full py-5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-xl hover:shadow-2xl hover:shadow-emerald-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3">
             {uploading ? <><Loader2 className="animate-spin"/> {t('saving')}</> : <><Save /> {t('save_product')}</>}
        </button>
